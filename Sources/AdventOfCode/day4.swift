...
enum Day4 {
    ...
    static func run(for input: [String]) throws -> Int {
        ...
        for line in input {
            ...
            if firstElfRange.contains(secondElfRange) || secondElfRange.contains(firstElfRange) {
                result += 1
            }
        }
        return result
    ...
    struct CampSectionsRange {
        ...
        func contains(_ anotherSection: CampSectionsRange) -> Bool {
            return anotherSection.start >= self.start
                && anotherSection.end <= self.end
        }
    }
}
